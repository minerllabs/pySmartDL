...
__all__ = ['SmartDL', 'utils', 'HashFailedException', 'CanceledException']
__version_major__ = 1
__version_minor__ = 3
__version_micro__ = 2
...
class HashFailedException(Exception):
    ...
    def __str__(self):
        return 'HashFailedException({}, got {}, expected {})'.format(self.filename, self.calculated_hash,
                                                                     self.needed_hash)
...
class SmartDL:
    ...
    def __init__(self, urls, dest=None, progress_bar=True, fix_urls=True, threads=5, timeout=5, logger=None,
    ...
    def start(self, blocking=None):
        ...
        if self.verify_hash and os.path.exists(self.dest):
            if _get_file_hash(self.hash_algorithm, self.dest) == self.hash_code:
                self.logger.info(
                    "Destination '%s' already exists, and the hash matches. No need to download." % self.dest)
        ...
        if len(args) > 1:
            self.logger.info(
                "Launching {} threads (downloads {}/thread).".format(len(args), utils.sizeof_human(bytes_per_thread)))
        else:
    ...
    def get_data(self, binary=False, bytes=-1):
        ...
        if self.status != 'finished':
            raise RuntimeError(
                "The download task must be finished in order to read the data. (current status is %s)" % self.status)
